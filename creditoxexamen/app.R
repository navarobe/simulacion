...
###### U I  ######
##################
ui <- navbarPage("Crédito por examen", id="principal",
                 ...
                 tabPanel("Ej. 2",
                          ...
                          sidebarLayout(
                              ...
                              mainPanel(
                                  ...
                                  column(width = 10, numericInput("clases_dist", "Número de clases", value = 100, min = 2),plotOutput("plot_dist"))
                              )
                          )
                 ),
    ...
    output$descarga_prueba<- downloadHandler(
        ...
        content = function(file){
            ...
            observeEvent(input$tipo_dist, {
                distribuciones<- if(input$tipo_dist==0){
                    c("Uniforme continua"= "1",
                      "Exponencial"="2",
                      "Gamma"="3",
                      "Weibull"="4",
                      "Normal"="5",
                ...
                updateSelectInput(session, "dist_dist", choices = distribuciones)
            })
            observeEvent(input$dist_dist,{
                ...
                if(input$dist_dist=="1"){
                    ...
                    x<-reactive({gamma_0mas(input$n_dist,input$gene,  input$alfa_gamma, input$beta_gamma)})
                    output$plot_dist<- renderPlot({
                        ggplot(data=x(), aes(x=x))+
                            geom_histogram(aes(y=..density.., fill="Función Empírica"),breaks = seq(0, max(x()), (max(x()))/input$clases_dist), alpha=.5, color="dodgerblue2")+
